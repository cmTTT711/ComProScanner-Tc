...
def disable_exit_program(monkeypatch):
    ...
    monkeypatch.setattr(
        "comproscanner._errors.BaseError.exit_program", lambda self: None
    )
...
def mock_heavy_ml_dependencies():
    ...
    with ExitStack() as stack:
        ...
        mock_tokenizer = None
        mock_model = None
        if "comproscanner.documents.csv_store" in sys.modules:
            mock_embeddings = stack.enter_context(
                patch("comproscanner.documents.csv_store.MultiModelEmbeddings")
            )
        if "comproscanner.evidence.rag.embeddings" in sys.modules:
            mock_tokenizer = stack.enter_context(
                patch("comproscanner.evidence.rag.embeddings.AutoTokenizer")
            )
            mock_model = stack.enter_context(
                patch("comproscanner.evidence.rag.embeddings.AutoModel")
            )
