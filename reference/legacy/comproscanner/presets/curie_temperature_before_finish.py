...
PROPERTY_KEYWORDS = {
    "candidate_patterns": [
        {
            "class": "DIRECT_TC",
            "pattern": r"(?i)(?<![A-Za-z])t\s*_?\s*c(?:urie)?(?![A-Za-z0-9])\s*(?:[=:≈~∼�]\s*|(?:is|of)\s+)[+-]?\d+(?:\.\d+)?(?:\s*(?:°\s*c|k)\b)?",
        },
        {
            "class": "CURIE",
            "pattern": r"(?i)\bcurie[-\s]+(?:temperature|point|transition)(?:[-\s]+transition)?\b",
        },
        {
            "class": "FE_PE_TRANSITION",
            "pattern": r"(?i)\b(?:ferroelectric|fe)\s*(?:-|–|—|/|to)\s*(?:paraelectric|pe)\s+(?:phase\s+)?transition\b|\bparaelectric\s*(?:-|–|—|/|to)\s*ferroelectric\s+(?:phase\s+)?transition\b",
        },
        {
            "class": "FE_TRANSITION_CONTEXT",
            "pattern": r"(?i)\b(?:normal\s+)?ferroelectric(?:\s+phase)?\s+transition(?:\s+temperature\b|\b.{0,120}?(?:\bT\s*_?\s*m\b|transition\s+temperature|dielectric\s+(?:maximum|peak)|[+-]?\d+(?:\.\d+)?\s*(?:°\s*c|k)\b))",
        },
        {
            "class": "TM_FE_CONTEXT",
            "pattern": r"(?i)(?:\bT\s*_?\s*m\b.{0,160}\b(?:ferroelectric|paraelectric)\b|\b(?:ferroelectric|paraelectric)\b.{0,160}\bT\s*_?\s*m\b)",
        },
    ],
    "exact_keywords": [
        "Curie temperature",
...
MATERIALS_DATA_IDENTIFIER_QUERY = (
    "Does the current article explicitly state a Curie temperature (Curie point, "
    "Tc, or T_C) for at least one identifiable material composition? Include "
    "current-work, background, comparison, and cited-material statements when the "
    "article makes the material-to-Tc relationship explicit. Answer 'yes' or 'no' "
    "with a single word. Do not count ambiguous material-value relationships or "
    "temperatures that are clearly synthesis/annealing/sintering/measurement/"
    "Néel/blocking/glass transition/decomposition temperatures. A Tm or dielectric "
    "peak qualifies only when its local context explicitly identifies a normal "
    "Curie or ferroelectric-to-paraelectric transition; exclude frequency-dependent "
    "relaxor peaks and generic dielectric maxima."
)

TC_EXTRACTOR_PRECISION_TASK_NOTES = [
    "ARTICLE SCOPE: Extract every material-to-Curie-temperature fact explicitly "
    "asserted by the article. This includes current-work samples, background or "
    "comparison materials, and facts attributed to cited publications. Do not use "
    "external knowledge or invent a relationship that the article does not state.",
    ...
    "dielectric maxima. When normal and relaxor samples coexist, extract only each "
    "material's locally supported normal Curie/FE-PE facts.",
    "BINDING: Bind every value to the exact material that the article associates "
    "with it. A constituent background fact may be extracted for that constituent, "
    "but must not be transferred to a composite. Likewise, keep a measured "
    "composite value bound to the composite rather than substituting a constituent.",
    "FINAL CHECK: For every proposed record, confirm that the article explicitly "
    "asserts the target transition and exact material-value binding. Remove the "
    "record if either check fails.",
]
...
def get_curie_temperature_flow_optional_args() -> dict:
    ...
    composition_property_extraction_task_notes = [
        "Extract the Curie temperature explicitly stated for each identifiable "
        "material composition in the current article.",
...
def get_curie_temperature_preset_definition() -> PropertyExtractionPreset:
    ...
    flow_args = get_curie_temperature_flow_optional_args()
