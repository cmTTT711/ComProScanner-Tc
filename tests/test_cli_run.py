...
cli_main = importlib.import_module("comproscanner.cli.main")
...
def test_run_raw_pdf_plan_is_inert(tmp_path, monkeypatch, capsys):
    ...
    assert main([
        "run", "--source", "manual_pdf", "--folder", str(pdfs),
        "--run-id", "planned-pdf", "--outputs", str(tmp_path / "outputs"),
    ]) == 0
    plan = json.loads(capsys.readouterr().out)
...
def test_run_network_source_is_blocked_before_side_effects(tmp_path):
    ...
    with pytest.raises(SystemExit, match="require --execute-network"):
        main([
            "run", "--source", "elsevier", "--run-id", "blocked-network",
            "--outputs", str(outputs), "--through", "prepare", "--execute-pipeline",
        ])
...
def test_run_processes_raw_pdf_in_isolated_workspace_and_restores_cwd(
    ...
    def fake_execute(plan, *, main_property_keyword, **kwargs):
        destination = (
            Path.cwd() / "results" / "extracted_data" / main_property_keyword
        )
        destination.mkdir(parents=True)
        pd.DataFrame([{
            "doi": "10.1/raw", "article_title": "Raw PDF",
            "abstract": "BiFeO3 has a Curie temperature of 1103 K.",
        }]).to_csv(
    ...
    assert main([
        "run", "--source", "manual_pdf", "--folder", str(pdfs),
        "--run-id", "raw", "--outputs", str(outputs),
        "--through", "prepare", "--execute-pipeline",
    ]) == 0
...
def test_run_resume_skips_completed_prepare_stage(tmp_path, monkeypatch):
    ...
    pd.DataFrame([{
        "doi": "10.1/resume", "article_title": "Resume",
        "abstract": "BiFeO3 has a Curie temperature of 1103 K.",
    }]).to_csv(source, index=False)
    outputs = tmp_path / "outputs"
    argv = [
        "run", "--csv", str(source), "--run-id", "resume",
        "--outputs", str(outputs), "--through", "prepare", "--execute-pipeline",
    ]
    assert main(argv) == 0
    ...
    monkeypatch.setattr(cli_main, "_prepare_evidence", forbidden)
...
def test_run_rejects_force_and_resume_together(tmp_path):
    ...
    with pytest.raises(ValueError, match="cannot be used together"):
        main([
            "run", "--csv", str(source), "--run-id", "conflict",
            "--through", "prepare", "--execute-pipeline", "--resume", "--force",
        ])
