...
def sample_xml_content():
    ...
    sample_xml_path = os.path.join(
        os.path.dirname(__file__), "../../fixtures", "elsevier_test.xml"
    )
...
def test_request_exception_uses_configured_retry_delay(elsevier_processor):
    response = MagicMock(status_code=200)
    with (
        patch(
            "comproscanner.documents.publishers.elsevier_processor.requests.get",
            side_effect=[requests.exceptions.RequestException("temporary failure"), response],
        ),
        patch(
            "comproscanner.documents.publishers.elsevier_processor.time.sleep"
        ) as sleep,
    ):
        assert elsevier_processor._send_request("10.test/retry") is response
...
def test_parse_response_with_error(elsevier_processor):
    ...
    with patch(
        "comproscanner.documents.publishers.elsevier_processor.logger.error"
    ) as mock_logger:
        root = elsevier_processor._parse_response(mock_response)
        assert root is None
...
def test_send_request_rate_limit(elsevier_processor, monkeypatch):
    ...
    with (
        ...
        patch(
            "comproscanner.documents.publishers.elsevier_processor.logger.critical"
        ) as mock_logger,
    ):
        monkeypatch.setattr(
            elsevier_processor,
...
def test_send_request_timeout(elsevier_processor, monkeypatch):
    ...
    with (
        ...
        patch(
            "comproscanner.documents.publishers.elsevier_processor.write_timeout_file"
        ) as mock_write,
        patch(
            "comproscanner.documents.publishers.elsevier_processor.logger"
        ) as mock_logger,
...
def test_process_with_timeout_handling(elsevier_processor, monkeypatch):
    ...
    with (
        ...
        patch(
            "comproscanner.documents.publishers.elsevier_processor.logger.debug"
        ) as mock_debug,
    ):
...
@patch("comproscanner.documents.publishers.elsevier_processor.logger.verbose")
@patch("comproscanner.documents.publishers.elsevier_processor.logger.debug")
@patch("comproscanner.documents.publishers.elsevier_processor.logger.info")
...
def test_keyboard_interrupt_handling(elsevier_processor, sample_df, monkeypatch):
    ...
    with (
        patch(
            "comproscanner.documents.publishers.elsevier_processor.tqdm"
        ) as mock_tqdm,
        patch(
            "comproscanner.documents.publishers.elsevier_processor.logger.error"
        ) as mock_logger,
...
def test_integration_flow(elsevier_processor, monkeypatch, sample_xml_content):
    ...
    with (
        ...
        patch.object(elsevier_processor, "df", sample_df, create=True),
        patch.object(elsevier_processor, "_send_request", return_value=mock_response),
        patch.object(
