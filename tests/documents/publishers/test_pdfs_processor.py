...
from comproscanner.documents.config import ArticleRelatedKeywords, DefaultPaths
from comproscanner._errors import ValueErrorHandler, KeyboardInterruptHandler
...
def test_init_valid_parameters(sample_property_keywords):
    ...
    assert (
        processor.track_pdfs_report_path
        == DefaultPaths("piezoelectric").PDF_PROCESSED_DOIS_FILENAME
    )
...
def test_mark_pdf_processed_writes_to_file(pdfs_processor_with_tracking):
    ...
    with patch("os.makedirs"), patch("builtins.open", m):
        pdfs_processor_with_tracking._mark_pdf_processed(
            "/some/path/paper1.pdf", "10.1234/test"
        )
...
@patch("glob.glob")
@patch(
    "comproscanner.documents.publishers.pdfs_processor.get_doi_from_crossref",
    return_value="",
)
def test_process_pdfs_allows_explicit_doi_optional_mode(
    ...
    row = pd.DataFrame({"doi": ["placeholder"], "is_property_mentioned": ["0"]})
    with (
        ...
        patch.object(pdfs_processor.article_store, "write_to_csv") as write_csv,
        patch("time.sleep"),
    ):
...
@patch("glob.glob")
@patch(
    "comproscanner.documents.publishers.pdfs_processor.get_paper_metadata_from_openalex",
...
def test_process_pdfs_with_doi(mock_metadata, mock_glob, pdfs_processor):
    ...
    with (
        ...
        patch.object(pdfs_processor.article_store, "write_to_csv"),
    ):
        pdfs_processor.process_pdfs()
...
def test_process_pdfs_skips_already_processed(mock_glob, pdfs_processor):
    ...
    with (
        patch(
            "comproscanner.documents.publishers.pdfs_processor.PDFsProcessor._load_processed_pdfs",
...
@patch("glob.glob")
@patch("comproscanner.documents.metadata.get_paper_metadata_from_openalex")
def test_process_pdfs_exception_handling(mock_metadata, mock_glob, pdfs_processor):
    """Test exception handling during PDF processing"""
    mock_glob.return_value = ["/test/path/file1.pdf", "/test/path/file2.pdf"]
    mock_metadata.return_value = ("Test Title", "Test Journal", "Test Publisher")
