...
def test_equal_values_under_different_experimental_conditions_stay_separate():
    random = replace(make_fact(), conditions={"microstructure": "randomly mixed"})
    trilayer = replace(
        make_fact(evidence="text_2"), conditions={"microstructure": "trilayer"}
    )
    assert merge_facts([random, trilayer, make_fact(evidence="text_3")]) == [
        random,
        trilayer,
        make_fact(evidence="text_3"),
    ]


def test_same_conditions_merge_regardless_of_key_order_and_preserve_diagnostics():
    first = replace(
        make_fact(),
        conditions={"method": "magnetization", "pressure": {"value": 1, "unit": "bar"}},
        processing_issues=("source_uncertain",),
    )
    second = replace(
        make_fact(evidence="table_1"),
        conditions={"pressure": {"unit": "bar", "value": 1}, "method": "magnetization"},
        processing_issues=("source_uncertain", "material_normalization_empty"),
    )
    merged = merge_facts([first, second])
    assert len(merged) == 1
    assert merged[0].conditions == first.conditions
    assert merged[0].evidence_ids == ("text_1", "table_1")
    assert merged[0].processing_issues == (
        "source_uncertain",
        "material_normalization_empty",
    )
