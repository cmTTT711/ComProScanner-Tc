...
_EXPLICIT_ACRONYM = re.compile(r"^(?P<name>.+?)\s*\((?P<alias>[A-Z][A-Z0-9-]{1,9})\)$")
...
def _merge_key(fact: Fact) -> tuple:
    ...
    return (
        ...
        *original[3:],
        json.dumps(fact.attributes, sort_keys=True, ensure_ascii=False, default=str),
        json.dumps(fact.conditions, sort_keys=True, ensure_ascii=False, default=str),
    )
    ...
    for fact in facts:
        ...
        reported_variants = tuple(
            dict.fromkeys(
                [
                    *(
                        existing.material_reported_variants
                        or (existing.material_reported,)
                    ),
        ...
        merged[key] = Fact(
            ...
            material_resolution=tuple(
                {
                    json.dumps(item, sort_keys=True, ensure_ascii=False): item
                    for item in (
                        *existing.material_resolution,
                        *fact.material_resolution,
                    )
                }.values()
            ),
            processing_issues=tuple(
                dict.fromkeys([*existing.processing_issues, *fact.processing_issues])
            ),
        )
