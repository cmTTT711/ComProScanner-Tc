...
def normalize_legacy_article_frame(
    ...
    *,
    ...
    # Preserve publisher metadata and parser sidecar locations outside fixed Article columns.
    extras = [key for key in frame.columns if key not in ARTICLE_CSV_COLUMNS]
    if extras:
        ...
        normalized["metadata_json"] = normalized.apply(metadata, axis=1)
    ...
    missing_full_text = (
        normalized["full_text"].fillna("").astype(str).str.strip().eq("")
    )
    if missing_full_text.any():
        normalized.loc[missing_full_text, "full_text"] = (
            normalized.loc[missing_full_text, section_columns]
            .fillna("")
            .astype(str)
            .agg("\n\n".join, axis=1)
            .str.strip()
