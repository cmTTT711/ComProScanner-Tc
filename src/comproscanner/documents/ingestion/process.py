...
def build_processing_plan(
    *,
    ...
    required = {
        variable for source in definitions for variable in source.credential_env
    }
    optional = {
        variable
    ...
    missing = tuple(
        sorted(variable for variable in required if not os.getenv(variable))
    )
    ...
    processor_sources = tuple(
        dict.fromkeys(
            "pdfs" if name in LOCAL_PDF_SOURCES else name for name in requested
        )
    )
...
def execute_processing_plan(
    ...
    *,
    ...
    if scanner_factory is None:
        ...
        scanner_factory = ArticleProcessor
    # Publisher processors expect a metadata frame even for an explicit DOI list.
    # Missing metadata stays empty; no hidden discovery/network request is made.
    if dois and plan.network_required:
        import pandas as pd

        metadata = Path("results") / f"{main_property_keyword}_metadata.csv"
        if not metadata.exists():
            metadata.parent.mkdir(parents=True, exist_ok=True)
            publisher = (
                plan.processor_sources[0] if len(plan.processor_sources) == 1 else ""
            )
            pd.DataFrame(
                [
                    {
    ...
    kwargs.update(
        {
            ...
            # Evidence tools, including vector retrieval, run after Article normalization.
        }
    )
