...
class PrepareIOPFiles:
    def __init__(self, main_property_keyword: str = None, logger=None):
        ...
        self.df = (
            pd.read_csv(self.metadata_csv_filename)
            if Path(self.metadata_csv_filename).is_file()
            else pd.DataFrame()
        )
    ...
    def _combine_xml_files(self, folder_path):
        ...
        for root, file in tqdm(
            ...
            try:
                ...
                modified_doi = self._get_modified_doi(source_path)
                if modified_doi:
                    self.document_directories[modified_doi] = str(
    ...
    def _process_zip_files(self, folder_path: str) -> int:
        ...
        # Retain extracted assets in the working copy for later figure loading.
        temp_dir = os.path.join(folder_path, "archive_assets")
        ...
        for zip_file in tqdm(zip_files, desc="Processing ZIP files"):
            try:
                ...
                # Keep the source archive, including non-XML assets.
        ...
        # Keep non-XML assets at their original relative locations for figure loading.
    ...
    def prepare_files(self):
        ...
        """
        original = Path(self.xml_folderpath).resolve()
        working = Path("sources/iop").resolve()
        if original != working:
            if working.is_relative_to(original):
                raise ValueError(
                    "IOP working directory must be outside the original input"
                )
            shutil.copytree(original, working, dirs_exist_ok=True)
            self.xml_folderpath = str(working)
        ...
        # Local XML is sufficient input: supplement missing discovery metadata
        # from the retained source itself, without contacting a metadata service.
        known = set(self.df.get("doi", pd.Series(dtype=str)).dropna().astype(str))
        rows = []
        for path in Path(self.xml_folderpath).glob("*.xml"):
            root = self._return_xml_root(str(path))
            if root is None:
                continue
            doi = root.xpath(
                './/*[local-name()="article-id"][@pub-id-type="doi"]/text()'
            )
            if not doi:
                continue
            doi = doi[0].strip()
            canonical = path.parent / (doi.replace("/", "_") + ".xml")
            ...
            rows.append(
                {
                    "doi": doi,
                    "article_title": text_of("article-title"),
                    "publication_name": text_of("journal-title"),
                    "metadata_publisher": "IOP",
                    "general_publisher": "iop",
                }
            )
            known.add(doi)
        if rows:
            self.df = pd.concat([self.df, pd.DataFrame(rows)], ignore_index=True)
            Path(self.metadata_csv_filename).parent.mkdir(parents=True, exist_ok=True)
            self.df.to_csv(self.metadata_csv_filename, index=False)
    ...
    def get_all_iop_dois(self) -> list:
        ...
        for file in os.listdir(self.xml_folderpath):
            if file.lower().endswith(".xml"):
                root = self._return_xml_root(str(Path(self.xml_folderpath) / file))
                doi = (
                    root.xpath(
                        './/*[local-name()="article-id"][@pub-id-type="doi"]/text()'
                    )
        ...
        return list(dict.fromkeys(iop_dois))
