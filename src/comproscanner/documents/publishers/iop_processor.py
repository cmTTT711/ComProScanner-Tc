...
from comproscanner.documents.prepare_iop import PrepareIOPFiles
...
class IOPArticleProcessor:
    ...
    def _get_iop_cdn_path_parts(self, root):
        issn = self._get_first_xpath_text(
            root, './/*[local-name()="journal-meta"]/*[local-name()="issn"][1]/text()'
        )
        volume = self._get_first_xpath_text(
            root, './/*[local-name()="article-meta"]/*[local-name()="volume"][1]/text()'
        )
        issue = self._get_first_xpath_text(
            root, './/*[local-name()="article-meta"]/*[local-name()="issue"][1]/text()'
        )
    ...
    def _download_iop_figure_from_content_cdn(
        ...
        for revision in revision_candidates:
            url = f"https://content.cld.iop.org/journals/{issn}/{volume}/{issue}/{elocation_id}/revision{revision}/{figure_filename}"
            try:
            ...
            if self._is_iop_no_such_key(resp):
                continue
            logger.warning(
                f"Unexpected response for IOP content CDN URL '{url}' (HTTP {resp.status_code})."
            )
        return None
    ...
    def _process_xml(self, root):
        ...
        all_sections = root.xpath('.//*[local-name()="body"]/*[local-name()="sec"]')
        if all_sections:
    ...
    def _extract_and_save_figures(self, root, doi: str, xml_dir: str):
        """
        Save all available IOP XML figures; caption matches are diagnostic only.
        ...
        try:
            ...
            if cdn_parts:
                ...
                logger.warning(
                    f"Could not find ISSN/volume/issue/elocation-id for {doi}. IOP content CDN figure fallback may fail."
                )
            ...
            for i, figure in enumerate(figures):
                ...
                if content:
                    ...
                    logger.warning(
                        f"Could not download IOP figure '{caption_id}' for {doi} from content CDN; caption saved to info.json only."
                    )
    ...
    def _process_articles(self):
        try:
            ...
            self.iop_folderpath = self.prepare_iop_files.xml_folderpath
                ...
                try:
                    ...
                    if doi in all_available_iop_dois:
                        ...
                        has_caption_keyword_match = self._extract_and_save_figures(
                            root,
                            doi,
                            self.prepare_iop_files.document_directories.get(
                                modified_doi, self.iop_folderpath
                            ),
                        )
