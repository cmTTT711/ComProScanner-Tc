...
from comproscanner.evidence.chunking import TextChunker
...
from comproscanner.evidence.providers import EquationEvidenceProvider
from comproscanner.evidence.providers import FigureEvidenceProvider
from comproscanner.evidence.providers import TableEvidenceProvider
from comproscanner.evidence.source_units import equation_units_from_chunks
from comproscanner.evidence.source_units import figure_units_from_manifest
from comproscanner.evidence.source_units import table_units_from_text
...
class EvidencePreparationPipeline:
    ...
    def __init__(
        ...
        *,
        ...
        modality_patterns: dict[str, tuple[str, ...]] | None = None,
    ):
        ...
        self.provider_names = tuple(
            dict.fromkeys(
                name.strip().casefold()
                for name in (
                    provider_names or ("rule_text", "table", "figure", "equation")
                )
            )
        )
        source_patterns = preset_patterns(property_keywords)
        modality_patterns = modality_patterns or {}
        rule_patterns = tuple(rule_patterns or ()) or source_patterns
        registry = provider_registry or EvidenceProviderRegistry()
        if provider_registry is None:
            registry.register(
                "rule_text", lambda: RuleTextEvidenceProvider(rule_patterns)
            )
            registry.register(
                "table",
                lambda: TableEvidenceProvider(
                    modality_patterns.get("table", source_patterns)
                ),
            )
            registry.register(
                "figure",
                lambda: FigureEvidenceProvider(
                    modality_patterns.get("figure", source_patterns)
                ),
            )
            registry.register(
                "equation",
                lambda: EquationEvidenceProvider(
                    modality_patterns.get("equation", source_patterns)
                ),
            )
    ...
    def prepare_text(
        ...
        *,
        ...
        full_text = (
            "" if full_text is None or str(full_text) == "nan" else str(full_text)
        )
        if full_text.strip():
            ...
            for section in TEXT_SECTIONS:
                ...
                sections[section] = (
                    "" if value is None or str(value) == "nan" else str(value)
                )
    ...
    def prepare_all(
        ...
        *,
        ...
        chunks, text_evidence = self.prepare_text(row, vector_matches=vector_matches)
        document_id = str(row["document_id"])
        tables = (
            table_units_from_text(document_id, row.get("tables", ""))
        ...
        figures = (
            figure_units_from_manifest(row.get("figures_manifest_path", ""))
